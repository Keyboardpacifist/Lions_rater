...
DATA_PATH = Path(__file__).resolve().parent.parent / "data" / "league_ol_all_seasons.parquet"
...
if len(df) == 0:
    st.warning(f"No {team_name} offensive linemen found for {selected_season}.")
    st.stop()
...
_yr = render_player_year_picker(
    ...
    team_col="team",
    key_prefix=f"ol_{player.get('player_id') or selected}",
)
...
NFL_SUM_COLS = {"off_snaps", "def_snaps", "snaps", "games", "games_played",
                ...
                "pos_run_plays", "penalties_total"}
# ── Trading-card visual ────────────────────────────────────────
_team_abbr = _yr["team_str"] if _yr["team_str"] else (player.get("recent_team") or "")
render_player_card(
    player_name=selected,
    position_label=(player.get("depth_position") or player.get("position") or "OL"),
    team_abbr=_team_abbr,
    season_str=_yr["season_str"],
    score=_view_score,
    stat_specs=OL_STAT_SPECS,
    view_row=view_row,
...
try:
    ...
    _season_pool = all_ol_full[all_ol_full["season_year"] == selected_season]
    _card_narrative = build_position_narrative(
        player_row=view_row, peer_pool=_season_pool,
        stat_labels=stat_labels, position_label="offensive linemen",
    )
except Exception:
    _card_narrative = None

_ol_pos = (player.get("depth_position") or player.get("position") or "OL")
_card_stats = [
    ("Snaps",   _safe_fmt(view_row.get("off_snaps")),
                _safe_fmt(view_row.get("snap_share"), "{:.0%}")),
    ("Games",   _safe_fmt(view_row.get("games_played")), ""),
    ("Pens",    _safe_fmt(view_row.get("penalties_total")), ""),
    ("Team Sk%", _safe_fmt(view_row.get("team_sack_rate"), "{:.1%}"), ""),
]
...
_render_card(
    ...
    preset_name=(st.session_state.ol_loaded_algo.get("name")
                  if st.session_state.get("ol_loaded_algo") else None),
    key_prefix=f"ol_{player.get('player_id') or selected}",
)

...
from lib_shared import apply_algo_weights, community_section, compute_effective_weights, get_algorithm_by_slug, inject_css, render_master_detail_leaderboard, render_player_stat_bar, render_player_year_picker, score_players
...
DATA_PATH = Path(__file__).resolve().parent.parent / "data" / "league_cb_all_seasons.parquet"
...
BUNDLES = {
    "coverage": {
        "label": "🛡️ Coverage",
        "description": "Does he lock down receivers? Passes defended and interceptions.",
        "why": "Think ball skills and coverage are what define a great CB? Crank this up.",
        "stats": {"passes_defended_per_game_z": 0.45, "interceptions_per_game_z": 0.55},
    },
    "tackling": {
        "label": "🎯 Tackling",
        "description": "Does he finish plays? Solo tackles and tackle rate.",
        "why": "Value corners who tackle well in the run game and on screens? Slide right.",
        "stats": {"solo_tackle_rate_z": 0.40, "tackles_per_snap_z": 0.40, "tfl_per_game_z": 0.20},
    },
    "playmaking": {
        "label": "💥 Playmaking",
        "description": "Does he force turnovers beyond just INTs? Forced fumbles and TFLs.",
        "why": "Want corners who create chaos, not just coverage? Slide right.",
        "stats": {"forced_fumbles_per_game_z": 0.50, "tfl_per_game_z": 0.50},
    },
}
DEFAULT_BUNDLE_WEIGHTS = {"coverage": 60, "tackling": 30, "playmaking": 30}

RADAR_STATS = ["solo_tackle_rate_z", "tackles_per_snap_z", "tfl_per_game_z", "forced_fumbles_per_game_z", "passes_defended_per_game_z", "interceptions_per_game_z"]
RADAR_INVERT = set()
RADAR_LABEL_OVERRIDES = {"solo_tackle_rate_z": "Solo tackle %", "tackles_per_snap_z": "Tackles/snap", "tfl_per_game_z": "TFLs", "forced_fumbles_per_game_z": "Forced fumbles", "passes_defended_per_game_z": "Pass defense", "interceptions_per_game_z": "Interceptions"}
...
if len(df) == 0:
    st.warning(f"No {team_name} cornerbacks found for {selected_season}.")
    st.stop()
...
if st.session_state.cb_loaded_algo:
    ...
    if st.sidebar.button("Clear loaded algorithm"):
        st.session_state.cb_loaded_algo = None
    ...
    with tier_cols[i]:
        ...
        if has_stats:
            checked = st.checkbox(
                ...
                value=(tier in st.session_state.cb_tiers_enabled),
                help=TIER_DESCRIPTIONS[tier],
                key=f"cb_tier_checkbox_{tier}",
            )
            if checked:
...
for bk, bundle in active_bundles.items():
    ...
    if f"cb_bundle_{bk}" not in st.session_state:
        st.session_state[f"cb_bundle_{bk}"] = DEFAULT_BUNDLE_WEIGHTS.get(bk, 50)
    bundle_weights[bk] = st.sidebar.slider(
        bundle["label"], 0, 100, step=5,
        key=f"cb_bundle_{bk}", label_visibility="collapsed",
        help=bundle.get("why", ""),
    )
...
with st.sidebar.expander("Want more control? Adjust individual stats"):
    advanced_mode = st.checkbox("Enable individual stat control", value=False, key="cb_advanced_toggle")
    if advanced_mode:
        ...
        for z_col in all_enabled_stats:
            ...
            w = st.slider(f"{tier_badge(stat_tiers.get(z_col, 2))} {label}", 0, 100, 50, 5, key=f"adv_cb_{z_col}", help=help_text if help_text else None)
...
all_cbs_full = load_cb_data()
player_career = all_cbs_full[all_cbs_full["player_id"] == player.get("player_id")] if "player_id" in all_cbs_full.columns else all_cbs_full[0:0]
...
_yr = render_player_year_picker(
    ...
    team_col="team",
    key_prefix=f"cb_{player.get('player_id') or selected}",
)
...
with c2:
    st.markdown("**Percentile profile vs. all league cornerbacks**")
    ...
    # ── Compare radar to another cornerback ────────────
    _radar_cmp_active = st.checkbox(
        "🔍 Compare radar to another cornerback",
        key=f"cb_radar_cmp_{player.get('player_id', selected)}",
        help="Stack a second player's radar polygon below this one, using the same year selection.",
    )
    if _radar_cmp_active:
        _pool = sorted(set(
            str(n) for n in all_cbs_full["player_display_name"].dropna().unique()
            if str(n).strip()
        )) if "player_display_name" in all_cbs_full.columns else []
        _default_cmp = next(
        ...
        if _default_cmp:
            _cmp_name = st.selectbox(
                "Comparison cornerback",
                options=_pool,
                index=_pool.index(_default_cmp),
                key=f"cb_radar_cmp_select_{player.get('player_id', selected)}",
            )
            if _cmp_name:
                _cmp_career = all_cbs_full[all_cbs_full["player_display_name"] == _cmp_name]
...
career_arc_section(
    ...
    position_label="cornerbacks",
)

st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
st.caption("Data via [nflverse](https://github.com/nflverse) · 2024 regular season · Compared against 124 CBs with 200+ snaps · Fan project, not affiliated with the NFL or Detroit Lions.")

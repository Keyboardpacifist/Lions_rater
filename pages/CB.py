...
BUNDLES = {
    "coverage": {
        ...
        "description": "Catch rate allowed, yards per target allowed, passer rating against.",
        "why": "Think the heart of CB play is suppressing what the QB can do when targeting you? Crank this up.",
        "stats": {
            "passer_rating_allowed_z": 0.35,
            "completion_pct_allowed_z": 0.25,
            "yards_per_target_allowed_z": 0.25,
            "passes_defended_per_game_z": 0.10,
            "interceptions_per_game_z": 0.05,
        },
    },
    "ball_hawking": {
        "label": "🦅 Ball hawking",
        "description": "Plays on the ball. PDs and INTs in volume.",
        "why": "Want CBs who turn targets into incompletions and turnovers? Slide right.",
        "stats": {
            "passes_defended_per_game_z": 0.45,
            "interceptions_per_game_z": 0.55,
        },
    },
    "tackling": {
        "label": "🎯 Tackling",
        "description": "Does he finish plays in the run game and on screens? Tackle reliability.",
        "why": "Value CBs who tackle well rather than dance around contact? Slide right.",
        "stats": {
            "solo_tackle_rate_z": 0.30, "tackles_per_snap_z": 0.30,
            "missed_tackle_pct_z": 0.30, "tfl_per_game_z": 0.10,
        },
    },
    "playmaking": {
        "label": "💥 Playmaking",
        "description": "Forced fumbles and TFLs — does he create chaos?",
        "why": "Want corners who create chaos, not just coverage? Slide right.",
        "stats": {"forced_fumbles_per_game_z": 0.50, "tfl_per_game_z": 0.50},
    },
}
DEFAULT_BUNDLE_WEIGHTS = {"coverage": 70, "ball_hawking": 40, "tackling": 30, "playmaking": 30}
...
render_nfl_player_banner(
    position="cb", player_name=selected, view_row=view_row,
    score=_view_score,
...
render_advanced_production(view_row, "cb", all_cbs_full,
                              season=_yr_for_panels)
render_movement_panel(selected, "cb", season=_yr_for_panels)
...
_card_narrative = make_card_narrative(view_row, all_cbs_full, "cb")
...
_card_stats = [
    ...
    ("Pass Rtg allowed", _safe_fmt(view_row.get("passer_rating_allowed"), "{:.1f}"), ""),
...
_render_card(
    ...
    position_label=(player.get("position") or "CB"),
    ...
    preset_name=(st.session_state.cb_loaded_algo.get("name")
                  if st.session_state.get("cb_loaded_algo") else None),
    key_prefix=f"cb_{player.get('player_id') or selected}",
    position_group="cb",
...
with tab_compare:
    ...
    render_player_comparison(
        player_row=view_row,
        player_name=selected,
        league_df=all_cbs_full,
        name_col="player_display_name",
        year_choice=year_choice,
        primary_score=_view_score,
        compute_comparison_score=_cb_score_of,
        radar_builder=build_radar_figure,
        stat_labels=stat_labels,
        stat_methodology=stat_methodology,
        key_prefix=f"cb_cmp_{player.get('player_id', selected)}",
        position_label="cornerback",
...
with tab_career:
    ...
    render_combine_chart(
        player_name=selected,
        position="CB",
        pool_positions=["CB", "DB"],
        workouts_path=_WORKOUTS_PATH,
        key=f"cb_combine_chart_{player.get('player_id', selected)}",
    )
    career_arc_section(
        player=player,

...
with c2:
    ...
    # ── Compare radar to another tight end ────────────
    _radar_cmp_active = st.checkbox(
        "🔍 Compare radar to another tight end",
        key=f"te_radar_cmp_{player.get('player_id', selected)}",
        help="Stack a second player's radar polygon below this one, using the same year selection.",
    )
    if _radar_cmp_active:
        _pool = sorted(set(
            str(n) for n in all_tes_full["player_display_name"].dropna().unique()
            if str(n).strip()
        ))
        _default_cmp = next(
        ...
        if _default_cmp:
            _cmp_name = st.selectbox(
                "Comparison tight end",
                options=_pool,
                index=_pool.index(_default_cmp),
                key=f"te_radar_cmp_select_{player.get('player_id', selected)}",
            )
            if _cmp_name:
                _cmp_career = all_tes_full[all_tes_full["player_display_name"] == _cmp_name]

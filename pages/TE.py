"""
TE Rater
Tight ends only. Z-scored within the TE pool — all TEs league-wide with 100+ offensive snaps.
"""
import json
...
# Per-stat raw-value formatting for radar benchmark hover
...
sort_label, sort_col, sort_ascending = metric_picker(TE_METRICS, key="te_metric_picker")
...
st.markdown("**How to read the score:** 0.00 = avg starting TE (z-scores baselined on top-32 by snaps). The percentile shows where this player ranks among all qualifying TEs (100+ snaps).")
...
selected = render_master_detail_leaderboard(
    ...
    leaderboard_caption=(
        "⚠️ = under 300 snaps. **Tgt%** = share of team's targets · "
        "**EPA/tgt** = Expected Points Added per target · "
        "**YAC/exp** = yards-after-catch above NGS expectation. "
...
# ── Split-season panel for traded TEs ──
all_tes_full = load_te_data()
season_stints = all_tes_full[
    (all_tes_full["player_id"] == player.get("player_id"))
    & (all_tes_full["season_year"] == selected_season)
].copy()
if len(season_stints) > 1:
    ...
    total_games = _safe_sum("games"); total_snaps = _safe_sum("off_snaps")
    total_rec = _safe_sum("receptions"); total_yds = _safe_sum("rec_yards"); total_tds = _safe_sum("rec_tds")
    total_targets = _safe_sum("targets")
    if "target_share" in season_stints.columns and "targets" in season_stints.columns:
        ts = season_stints["target_share"]; tg = season_stints["targets"]
        mask = ts.notna() & tg.notna() & (ts > 0)
        season_tgt_share = (total_targets / (tg[mask] / ts[mask]).sum()) if mask.any() and (tg[mask] / ts[mask]).sum() > 0 else float("nan")
    else:
        season_tgt_share = float("nan")
    season_epa = _weighted_mean("epa_per_target", "targets")
    season_yac = _weighted_mean("yac_above_exp", "receptions")
    split_rows.append({
        "Team": f"**Total ({selected_season})**",
        "Games": _fmt_int(total_games), "Snaps": _fmt_int(total_snaps),
        "Rec": _fmt_int(total_rec), "Yds": _fmt_int(total_yds), "TDs": _fmt_int(total_tds),
        "Tgt%": _fmt_pct(season_tgt_share),
        "EPA/tgt": _fmt_signed(season_epa, 2),
        "YAC/exp": _fmt_signed(season_yac, 1),
    })
...
_render_card(
    ...
    position_label=(player.get("position") or "TE"),
    ...
    preset_name=(st.session_state.te_loaded_algo.get("name")
                  if st.session_state.get("te_loaded_algo") else None),
    key_prefix=f"te_{player.get('player_id') or selected}",
    position_group="te",
...
_season_pool_te = all_tes_full[all_tes_full["season_year"] == selected_season]
_top32_te = _season_pool_te.sort_values("off_snaps", ascending=False).head(32)
_radar_bench = {z: _top32_te[z].mean() for z in RADAR_STATS
                  if z in _top32_te.columns and _top32_te[z].notna().any()}
_radar_bench_raw = {}
for z in RADAR_STATS:
    raw_col = RAW_COL_MAP.get(z)
    if raw_col and raw_col in _top32_te.columns and _top32_te[raw_col].notna().any():
        _radar_bench_raw[z] = _top32_te[raw_col].mean()


def _te_score_of(row):
...
with tab_profile:
    ...
    with c2:
        ...
        st.caption("Solid blue = this player. Dashed gray = top-32 starter average. Compared to TEs only, not WRs.")
...
with tab_compare:
    ...
    _te_team_abbr = (player.get("recent_team") or "")
    render_player_comparison(
        player_row=view_row,
        player_name=selected,
        league_df=all_tes_full,
        name_col="player_display_name",
        year_choice=year_choice,
        primary_score=_view_score,
        compute_comparison_score=_te_score_of,
        ...
        key_prefix=f"te_cmp_{player.get('player_id', selected)}",
        position_label="tight end",
        theme=_theme(_te_team_abbr),
    )
...
with tab_career:
    ...
    render_combine_chart(
        ...
        key=f"te_combine_chart_{player.get('player_id', selected)}",
    )
    career_arc_section(
        player=player,
...
st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
st.caption("Data via [nflverse](https://github.com/nflverse) · NGS tracking data · regular season only · Z-scored against league-wide TEs with 100+ offensive snaps · Fan project, not affiliated with the NFL or any team.")

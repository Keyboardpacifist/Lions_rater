...
st.set_page_config(page_title="Lions DT Rater", page_icon="🦁", layout="wide", initial_sidebar_state="expanded")
inject_css()
...
BUNDLES = {
    ...
    "run_defense": {"label": "🛡️ Run defense", "description": "Stops the run. Penetrates the backfield. Makes solo tackles.", "stats": {"tfl_per_game_z": 0.40, "solo_tackle_rate_z": 0.30, "tackles_per_snap_z": 0.30}},
    "playmaking": {"label": "💥 Playmaking", "description": "Forces fumbles and bats passes. Game-changing disruption.", "stats": {"forced_fumbles_per_game_z": 0.50, "passes_defended_per_game_z": 0.50}},
}
...
SCORE_EXPLAINER = """
**What this number means.** Weighted average of z-scores — 0 is league-average DT, +1 is one SD above, −1 is one SD below.
...
**DT population:** 2024 regular season, z-scored against all DTs league-wide with 200+ defensive snaps.
"""
...
st.markdown("**Build your own algorithm.** Drag the sliders to weight what you value, and watch the Lions defensive tackles re-rank in real time. _No 'best DT' — just **your** best DT._")
st.caption("2024 regular season • Z-scores vs all DTs league-wide (200+ snaps) • Every stat has a methodology popover")

try: df = load_data()
except FileNotFoundError: st.error(f"Couldn't find DTs data at {DATA_PATH}."); st.stop()
...
if st.session_state.dt_loaded_algo:
    ...
    if st.sidebar.button("Clear loaded algorithm"): st.session_state.dt_loaded_algo = None
...
for i, tier in enumerate([1, 2, 3, 4]):
    with tier_cols[i]:
        checked = st.checkbox(f"{tier_badge(tier)} {TIER_LABELS[tier]}", value=(tier in st.session_state.dt_tiers_enabled), help=TIER_DESCRIPTIONS[tier], key=f"dt_tier_checkbox_{tier}")
...
if not advanced_mode:
    ...
    for bk, bundle in active_bundles.items():
        ...
        if f"dt_bundle_{bk}" not in st.session_state: st.session_state[f"dt_bundle_{bk}"] = DEFAULT_BUNDLE_WEIGHTS.get(bk, 50)
        bundle_weights[bk] = st.sidebar.slider(bundle["label"], 0, 100, step=5, key=f"dt_bundle_{bk}", label_visibility="collapsed")
    for bk in BUNDLES:
    ...
    for z_col in all_enabled_stats:
        ...
        w = st.sidebar.slider(f"{tier_badge(tier)} {label}", 0, 100, 50, 5, key=f"adv_dt_{z_col}", help="\n\n".join(help_parts) if help_parts else None)
        if w > 0: effective_weights[z_col] = w
    bundle_weights = {bk: 0 for bk in BUNDLES}

st.markdown("### Who's in the pool?")
st.caption("All Lions DTs with 200+ defensive snaps in 2024. Z-scores computed against all qualified DTs league-wide.")
dts = df.copy()
if len(dts) == 0: st.warning("No DTs found."); st.stop()
...
st.caption("⚠️ 2024 regular season. Z-scored against all DTs with 200+ defensive snaps league-wide.")

display_df = pd.DataFrame({
    "Rank": ranked.index, "": ranked.get("def_snaps", pd.Series([0]*len(ranked))).apply(sample_size_badge),
    "Player": ranked["player_name"], "Team": ranked.get("team", "—"),
...
selected = st.selectbox("Pick a DT to see their breakdown", options=ranked["player_name"].tolist(), index=0)
...
with c1:
    ...
    st.caption(f"**{team}** · {int(player.get('games') or 0)} games · {int(player.get('def_snaps') or 0)} snaps")
...
with c2:
    st.markdown("**DT profile** (percentiles vs. league DTs)")
    fig = build_radar_figure(player, stat_labels, stat_methodology)
    if fig: st.plotly_chart(fig, use_container_width=True)
    else: st.caption("No radar data available.")
    st.caption("Each axis shows where this DT ranks among all qualified DTs league-wide. 50 = median.")
...
st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
st.caption("Data via [nflverse](https://github.com/nflverse) • 2024 regular season • Z-scored against 105 DTs with 200+ snaps • Fan project, not affiliated with the NFL or Detroit Lions.")

...
gp.render_team_gas_section("dt", selected_team, selected_season,
                              title=f"GAS Score · {team_name} DTs")
st.markdown("---")
...
selected = render_master_detail_leaderboard(
    ...
    leaderboard_caption=(
        ...
        "**Pressures** = PFR pressures · "
        "**Press rate** = pressures / pass plays defended while on field. "
...
_yr = render_player_year_picker(
    ...
    key_prefix=f"dt_{player.get('player_id') or selected}",
)
...
render_nfl_player_banner(
    position="dt", player_name=selected, view_row=view_row,
    score=_view_score,
...
render_advanced_production(view_row, "dt", all_dts_full,
                              season=_yr_for_panels)
render_movement_panel(selected, "dt", season=_yr_for_panels)
...
_card_narrative = make_card_narrative(view_row, all_dts_full, "dt")
...
_render_card(
    ...
    position_label=(player.get("position") or "DT"),
    ...
    preset_name=(st.session_state.dt_loaded_algo.get("name")
                  if st.session_state.get("dt_loaded_algo") else None),
    key_prefix=f"dt_{player.get('player_id') or selected}",
    position_group="dt",
...
_season_pool_dt = all_dts_full[all_dts_full["season_year"] == selected_season] if "season_year" in all_dts_full.columns else all_dts_full
_snap_col_dt = "def_snaps" if "def_snaps" in _season_pool_dt.columns else "off_snaps"
_top32_dt = _season_pool_dt.sort_values(_snap_col_dt, ascending=False).head(32)
_radar_bench = {z: _top32_dt[z].mean() for z in RADAR_STATS if z in _top32_dt.columns and _top32_dt[z].notna().any()}
_radar_bench_raw = {}
for z in RADAR_STATS:
    raw_col = RAW_COL_MAP.get(z)
    if raw_col and raw_col in _top32_dt.columns and _top32_dt[raw_col].notna().any():
        _radar_bench_raw[z] = _top32_dt[raw_col].mean()


def _dt_score_of(row):
...
with tab_compare:
    ...
    render_player_comparison(
        player_row=view_row,
        player_name=selected,
        league_df=all_dts_full,
        name_col="player_display_name",
        year_choice=year_choice,
        primary_score=_view_score,
        compute_comparison_score=_dt_score_of,
        ...
        key_prefix=f"dt_cmp_{player.get('player_id', selected)}",
        position_label="defensive tackle",
...
with tab_career:
    ...
    render_combine_chart(
        player_name=selected,
        position="DT",
        pool_positions=["DT", "NT", "DL"],
        workouts_path=_WORKOUTS_PATH,
        key=f"dt_combine_chart_{player.get('player_id', selected)}",
    )
    career_arc_section(
        player=player,

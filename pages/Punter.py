...
render_nfl_player_banner(
    position="p", player_name=selected, view_row=view_row,
    score=_view_score,
...
try:
    ...
    _season_pool = all_punters_full[all_punters_full["season_year"] == selected_season] if "season_year" in all_punters_full.columns else all_punters_full
    _card_narrative = build_position_narrative(
        player_row=view_row, peer_pool=_season_pool,
        stat_labels=stat_labels, position_label="punters",
    )
except Exception:
    _card_narrative = None

_card_stats = [
    ("Punts", _safe_fmt(view_row.get("punts")), ""),
    ("Avg",   _safe_fmt(view_row.get("avg_distance"), "{:.1f}"),
              _safe_fmt(view_row.get("avg_net"), "{:.1f} net")),
    ("In 20%", _safe_fmt(view_row.get("inside_20_rate"), "{:.1%}"), ""),
    ("TB%",   _safe_fmt(view_row.get("touchback_rate"), "{:.1%}"), ""),
]
...
_render_card(
    ...
    preset_name=(st.session_state.punter_loaded_algo.get("name")
                  if st.session_state.get("punter_loaded_algo") else None),
    key_prefix=f"punter_{player.get('player_id') or selected}",
    position_group="punter",
...
_season_pool_p = all_punters_full[all_punters_full["season_year"] == selected_season] if "season_year" in all_punters_full.columns else all_punters_full
_radar_bench = {z: _season_pool_p[z].mean() for z in RADAR_STATS if z in _season_pool_p.columns and _season_pool_p[z].notna().any()}
_radar_bench_raw = {}
for z in RADAR_STATS:
    raw_col = RAW_COL_MAP.get(z)
    if raw_col and raw_col in _season_pool_p.columns and _season_pool_p[raw_col].notna().any():
        _radar_bench_raw[z] = _season_pool_p[raw_col].mean()
...
with tab_compare:
    ...
    render_player_comparison(
        player_row=view_row,
        player_name=selected,
        league_df=all_punters_full,
        name_col="player_display_name",
        year_choice=year_choice,
        primary_score=_view_score,
        compute_comparison_score=_p_score_of,
        ...
        key_prefix=f"p_cmp_{player.get('player_id', selected)}",
        position_label="punter",

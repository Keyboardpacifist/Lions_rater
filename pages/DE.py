"""
Lions DE Rater — Edge Rushers page
====================================
Lions defensive ends ranked using league-wide z-scores.
9 stats across 3 bundles: Pass Rush, Run Defense, Playmaking.
"""
...
@st.cache_data
def load_des_data(): return pl.read_parquet(DATA_PATH).to_pandas()
@st.cache_data
def load_des_metadata():
...
BUNDLES = {
    "pass_rush": {"label": "🔥 Pass rush", "description": "Gets to the QB. Sacks, hits, pressures, and backfield penetration.", "stats": {"sacks_per_game_z": 0.30, "qb_hits_per_game_z": 0.25, "pressure_rate_z": 0.25, "tfl_per_game_z": 0.20}},
    "run_defense": {"label": "🛡️ Run defense", "description": "Sets the edge. Makes solo tackles. Stays disciplined.", "stats": {"solo_tackle_rate_z": 0.50, "tackles_per_snap_z": 0.50}},
    "playmaking": {"label": "💥 Playmaking", "description": "Forces fumbles, bats passes, and picks off the QB.", "stats": {"forced_fumbles_per_game_z": 0.40, "passes_defended_per_game_z": 0.35, "interceptions_per_game_z": 0.25}},
}
DEFAULT_BUNDLE_WEIGHTS = {"pass_rush": 70, "run_defense": 40, "playmaking": 40}
...
**DE population:** z-scores computed against all DEs league-wide with 2+ qualified seasons (200+ defensive snaps/season) from 2016-2024. Career-weighted by snap count.
"""
...
st.markdown("**Build your own algorithm.** Drag the sliders to weight what you value, and watch the Lions edge rushers re-rank in real time. _No 'best DE' — just **your** best DE._")
st.caption("Lions DEs shown • Z-scores computed against league-wide DEs (200+ snaps/season) • Every stat has a methodology popover")

try: df = load_des_data()
except FileNotFoundError: st.error(f"Couldn't find DEs data at {DATA_PATH}."); st.stop()

meta = load_des_metadata()
...
st.caption("All Lions DEs with 200+ defensive snaps in a season. Z-scores computed against league-wide DEs.")
des = df.copy()
...
st.caption("⚠️ DEs with fewer seasons have noisier scores. 🔴 = 1 season, 🟡 = 2-3 seasons.")
...
with c1:
    ...
    st.caption(f"**{team}** · {int(player.get('seasons') or 0)} seasons · {int(player.get('total_games') or 0)} games · {int(player.get('total_snaps') or 0)} snaps")
...
with c2:
    ...
    st.caption("Each axis shows where this DE ranks among all qualified DEs league-wide. 50 = median.")

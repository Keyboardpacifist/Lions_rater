"""Build Safety GAS Score table.

Output: data/safety_gas_seasons.parquet
"""
...
MASTER = REPO / "data" / "league_s_all_seasons.parquet"
OUT = REPO / "data" / "safety_gas_seasons.parquet"

from lib_safety_gas import compute_safety_gas  # noqa: E402


SAFETY_POSITIONS = {"S", "SS", "FS", "SAF", "DB"}


def main() -> None:
    print("→ loading Safety master...")
    master = pd.read_parquet(MASTER)
    print(f"  rows: {len(master):,}")
    print(f"  positions: {master['position'].value_counts().to_dict()}")

    before = len(master)
    master = master[master["position"].isin(SAFETY_POSITIONS)].copy()
    print(f"  filtered to safety/DB: {len(master):,}  "
    ...
    graded = compute_safety_gas(master)
    ...
    print()
    print("=== Top 15 Safety by GAS, 2024 (≥600 def_snaps) ===")
    cols = ["player_display_name", "team", "position", "def_snaps",
            "gas_score", "gas_label", "gas_confidence",
            "gas_coverage_grade", "gas_run_support_grade",
            "gas_ball_production_grade", "gas_playmaking_grade"]
    ...
    print(f"Safety YoY r (≥500 snaps both years): "
